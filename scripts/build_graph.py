...
from graph_builder import extract_graph_elements

ARCHIVO_ORIGEN = os.path.join(BASE_DIR, "data", "EXPORT_SIPA_clean.csv")
DIR_GRAFO = os.path.join(BASE_DIR, "data", "graph")

print("=== INICIANDO CONSTRUCCIÓN DEL GRAFO DE CONOCIMIENTO (VRID-AlumniUC) ===")
t0 = time.time()
...
print(f"\n[1/3] Cargando dataset limpio desde {ARCHIVO_ORIGEN}...")
df = pd.read_csv(ARCHIVO_ORIGEN, low_memory=False)
print(f"-> Cargadas {len(df):,} filas y {len(df.columns)} columnas.")

print("\n[2/3] Extrayendo Nodos y Relaciones del Grafo...")
graph = extract_graph_elements(df)

os.makedirs(DIR_GRAFO, exist_ok=True)

print("\n[3/3] Guardando tablas CSV del Grafo en data/graph/...")
resumen_nodos = {}
resumen_aristas = {}
...
print("\n=== RESUMEN DEL GRAFO DE CONOCIMIENTO GENERADO ===")
print("\n--- NODOS EXTRAÍDOS ---")
...
print(f"\nUbicación de archivos CSV : {DIR_GRAFO}")
